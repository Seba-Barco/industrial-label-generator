...
import etiqueta_individual_doble
import etiqueta_caja
...
def check_and_create_directories():
    ...
    ** Main Function **
    ...
    if not os.path.exists(path_to_folder_label_editor_db):
        ...
        os.makedirs("Etiquetas Generadas")
...
def open_label_file(path, filename):
    ...
    ** Secondary Function **
    ...
    button_generate_pdf_files.configure(state="normal")
...
def new_label():
    ...
    button_generate_pdf_files.configure(state="disabled")
...
# ABAJO DE ESTO ESTA EL QUILOMBO DE CÓDIGO SUCIO
...
def check_and_enable_save_button(*args):
    # Check if all Entry fields have data
    all_fields_have_data = all(entry.get() for entry in input_fields)
...
def generate_labels():
    data_derivada = input_fields[1].get()
    data_codigo = input_fields[2].get()
    data_descripcion = input_fields[3].get()
    data_peso_unitario = input_fields[4].get()
    data_cables_por_caja = input_fields[5].get()
    data_cajas_por_pallet = input_fields[6].get()
    data_fecha = input_fields[7].get()
    # Calculated data for label generation
    data_peso_caja = float(data_peso_unitario) * int(data_cables_por_caja)
    data_peso_pallet = data_peso_caja * int(data_cajas_por_pallet)
    data_cables_por_pallet = int(data_cables_por_caja) * int(data_cajas_por_pallet)
    # Calculated data for final message
    data_cantidad_etiquetas_individuales = str(data_cables_por_pallet)  # Fix this, it is not the real TOTAL value, just the value per pallet
    data_cantidad_etiquetas_caja = str(data_cables_por_pallet // int(data_cables_por_caja))
    # Image path
    ...
    label_individual = [data_codigo,
                        ...
                        "       UTM: 1",
                        "Cantidad: 1",
                        "DERIVADA: " + data_derivada,
                        "Peso: " + data_peso_unitario + " kg"]

    label_caja = ["BRAND",
                  data_codigo,
                  data_descripcion,
                  "UTM: 1",
                  "Cantidad: " + data_cables_por_caja,
                  "Pres:",
                  "W: 000   X: 000   Y: 00" + data_cables_por_caja + "   Z: 001",
                  "Derivada: " + data_derivada,
                  "Peso: " + str(data_peso_caja) + " kg",
                  "Fecha: " + data_fecha]

    label_pallet = [image_path,
                    "BRAND INFORMATION",
                    "Para Empresa: BRAND INFORMATION",
                    "Pedido/Derivada: " + data_derivada,
                    data_codigo,
                    data_descripcion,
                    "UTM: 1",
                    "Cantidad: " + str(data_cables_por_pallet),
                    "Pres:",
                    "W: 0" + str(data_cables_por_pallet) + "   X: 000   Y: 000   Z: 001",
                    "Peso pallet: " + str(data_peso_pallet) + " kg",
                    "Proveedor: BRAND",
                    "Fecha: " + data_fecha]


    etiqueta_individual_doble.create_individual_label_pdf(label_individual)
    etiqueta_caja.create_box_label_pdf(label_caja)
    ...
    messagebox.showinfo("Generación Exitosa",
                        ...
                        "Individuales: " + data_cantidad_etiquetas_individuales +
                        "\nCaja: " + data_cantidad_etiquetas_caja +
                        "\nPallet: 4" +
                        "\n\n" +
                        "Multiplicar cada valor por la cantidad de pallets que se van a preparar.")
...
button_generate_pdf_files = tk.Button(root, text="Generar Archivos PDF", state="disabled", command=generate_labels)

# Create 8 text fields with input fields to the right
text_field_data = [
...
text_fields = []
input_fields = []
# The range has to be 8 because of the 8 text fields. We count from 1
for i in range(8):
    text_field = tk.Label(root, text=text_field_data[i])
    input_field = tk.Entry(root)
    text_fields.append(text_field)
    input_fields.append(input_field)
    # Bind to KeyRelease event
...
button_generate_pdf_files.grid(row=0, column=2, pady=(10, 10))
...
# Buttons
button_modify.grid(row=10, column=0, padx=10, pady=10, columnspan=2)
button_save.grid(row=10, column=1, pady=10, columnspan=2)
